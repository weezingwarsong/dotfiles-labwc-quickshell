...
| `mpris-toggle.sh` | Writes `"toggle"` to the `/tmp/qs-mpris-toggle` FIFO (only if the FIFO exists) to pin the MPRIS panel open, or unpin/close it. | `labwc/rc.xml` — `W-2` keybind |
